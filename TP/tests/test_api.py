...
FAKE_TRIANGLES_BYTES = (
    ...
    struct.pack("!ffffff", 0.0, 0.0, 1.0, 0.0, 1.0, 1.0) + #3 points seulement
...
#test le scénario A->B->C->D, tout fonctionne
    #scénario A: le Client appelle
    #scénario B: mock l'appel au PointSetManager pour qu'il réussiss
    mocker.patch(
    ...
    #scénario C: mock le calcul (le "cerveau")
    mocker.patch(
    ...
    #va lever 'NotImplementedError' dans app.py
    response = client.get(f"/triangulation/{VALID_UUID}")

    #scénario D - Vérifier la réponse
...
#test le Cas 404 (ID inconnu)
    #scénario B: mock l'appel pour qu'il lève une erreur HTTP 404
    mocker.patch(
        "triangulator.app.manager_client.fetch_pointset_from_manager",
...
def test_api_pointsetmanager_panne_reseau(client, mocker):
    #test le Cas 503 (panne réseau ou URLError)
    # Scénario B: Mock l'appel pour qu'il lève une URLError
    mocker.patch(
        "triangulator.app.manager_client.fetch_pointset_from_manager",
...
def test_api_pointsetmanager_donnees_corrompues(client, mocker):
    #test le Cas 500 (données binaires corrompues)
    #scénario B: mock l'appel (réussit)
    mocker.patch(
        "triangulator.app.manager_client.fetch_pointset_from_manager",
        return_value=b"\xDE\xAD\xBE\xEF" #données corrompue
    )
    ...
    #scénario C: L'app doit attraper l'erreur de 'binary_to_pointset'
    response = client.get(f"/triangulation/{VALID_UUID}")
...
def test_api_triangulation_interne_echoue(client, mocker):
    #test le Cas 500 (l'algorithme interne plante)
    #mock les premiéres étapes pour qu'elles réussissent
    mocker.patch(
        "triangulator.app.manager_client.fetch_pointset_from_manager",
        return_value=FAKE_POINTSET_BYTES
    )
    ...
    #scénario C: mock le "cerveau" pour qu'il lève une exception
    mocker.patch(
        "triangulator.app.core.compute_triangulation",
...
def test_api_invalid_uuid_format(client):
    ...
    """
    #scénario A: appel avec un ID non-UUID
    response = client.get("/triangulation/ID-PAS-UN-UUID")

    #scénario B: Vérification (Flask renvoie 404, pas 400, pour un type d'URL)
    assert response.status_code == 404